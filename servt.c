...
void server_init( int port )
{
  ...
  //First non-human player assigned first connecting program on socket
  for(i = 0; i < 2; i++) {
  	//Wants connections for every non-human player
    if( !is_human[i] ) {
      int tcp_no_delay = 1;
      ...
      //Agents open in/out files for writing to their terminal and getting input
      //Will be used on server_step when called because they are non-human inputs
      agent_fd[i]  = client;
               ...
               )
{
  ...
  //For second and third moves we print more previous moves to player?
  //We always update the most previous move to the player however
  if ( m == 2 ) { // second move
  ...
  if( i > 0 ) {
  	//Check if indeed a move was scanned from agent_output
  	//This scanned move is presumabley placed in the move array (pointer so updated)
    ...
    //If move was made, then make_move with given move
    if( move_scanned ) {
...
void play_games( int num_games, int move[] )
{
  ...
  //Board is 10x10 sized because we are indexing 1-9! 
  //Simplifies understanding
  int board[10][10];

  //Game status is a returned (ILLEGAL, STILLPLAYING, WON, or DRAW) based on move made
  int game_status;
  ...
  for( game=0; game < num_games; game++ ) {
    ...
    //Note that moves are made in this fashion:
    //move[x]move[y] means board x, position y on board x
    //Hence move[m-1] is used because the previous move chosen position
    //Will determine which board to play on!

    //This is why for the first move we use index 0 to choose inital board
    //Otherwise for human/non-humans they need to put two inputs for first move edge case
    ...
    //Any status other than still_playing (WON, DRAW, or ILLEGAL will end game)
    while( m < MAX_MOVE && game_status == STILL_PLAYING ) {
      //print_board( stdout,board,move[m-1],move[m] );
      m++;
      //Swap player for every move
      player = !player;

      //If current player was assigned to human via -x -o flag use human_step for input
      if( is_human[player] ) {
        game_status =  human_step( player,m,move,board );
      }
      //Else use server_step to make decision
      else {
...
int main(int argc, char *argv[])
{
  ...
  //If either player is non-human then initialise server on port
